...
class AnchorFreeHead(nn.Module):
    ...
    def forward_test(self, feat_list, mask_list, **kwargs):
        ...
        for l, (feat, mask) in enumerate(zip(feat_list, mask_list)):
            ...
            cls_pred_temp = self.cls_head(cls_feat)
            cls_pred.append(cls_pred_temp)
            # print(cls_pred_temp.shape)
            # print("------------------------------------------")
            reg_pred.append(F.relu(self.scale[l](self.reg_head(reg_feat))))
    ...
    def get_valid_proposals_scores(self, points, reg_pred, cls_pred, mask_list):
        ...
        masked_class = [9, 10, 11, 12, 13, 15, 20, 29, 34, 35, 39, 48, 49, 65, 68, 69, 78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 105, 111, 115, 117, 128, 129, 131, 146, 148, 149, 154, 155, 156]
        for cls in masked_class:
            scores[:, :, cls] = 0
